...
## Plugins for languages converting 语言转换相关插件

### Universal Lyric Fliiling Plugin 万能填词插件

> [!WARNING]
> (WIP)Because I forgot the development progress, support for some languages may not be complete yet. English are not supported yet.  
> (开发中)由于我忘记了开发进度，部分语言的支持可能尚未完成。暂时不支持英语。  
...
Building on the [General cross-lingual Job Plugin(Chinese Page)](https://www.bilibili.com/read/cv7732403/) created by [atonyxu](https://github.com/atonyxu), we have modified it into another version (referred to as SE, the second edition). This new version allows the dictionary to only provide simple conversion rules, while leaving the checking of unsupported CV pronunciations and phoneme splitting(WIP) to the plugin automatically.  
我们在[atonyxu](https://github.com/atonyxu)制作的[通用跨语种插件](https://www.bilibili.com/read/cv7732403/)的基础上再次修改了一版插件（称之为SE，第二版），让字典仅提供简单的转换规则，将不支持的CV发音与拆音（开发中）交由插件自动检查。
